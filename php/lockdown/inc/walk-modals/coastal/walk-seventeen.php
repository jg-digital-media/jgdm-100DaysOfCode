<!--<p> <?php echo "Walk 17";  ?> </p> -->

<article class="walk">

    <div> 

        <h3><?php echo $trailTitleSeventeen; ?></h3>

        <p>Blackhall rocks is one of the nicest most serene spots in the east coast of England on a warm day.</p>

    </div>

    <div>
        <img src = "img/walk_homepage/walk-seventeen.jpg" alt = "<?php echo $trailTitleSeventeen; ?>" title="<?php echo $trailTitleSeventeen; ?>" />

        <!-- opens modal -->
        <a href="#" class="walk-btn modal_btn" id="modal_btn_seventeen" title="Open Route">Open Route</a>
        
        <div id="modal-area-seventeen" class="modal">

            <div id="walk-modal-seventeen">

                <div class="modal-content">
                
                    <span class="close">&times;</span>

                    <h3>Trail Summary</h3>
                    
                    <ol>
                        <li>Blackhall Rocks:  From the car park turn right and follow the path straight on.</li>
                        <li>The coastline will be freely visible on your right.</li>
                        <li>.</li>
                        <li>.</li>
                    </ol>

                    <div class="lightbox-slider">

                        <!-- TEST FILE PATHS -->

                        <!-- walks project website URLS -->
                        <a href="https://jg-photography.s3.eu-west-2.amazonaws.com/lockdown_walks/blackhall-rocks/IMG_20200528_115618.jpg" data-lightbox="walk-seventeen" data-title="Follow the straight path that goes toward the coast line." class="lightbox-link"> 
                            <div class="follow-trail">Follow the Trail</div> 
                        </a>
                        <a href="https://jg-photography.s3.eu-west-2.amazonaws.com/lockdown_walks/blackhall-rocks/IMG_20200528_115745.jpg" data-lightbox="walk-seventeen" data-title="At this point the coast line will be on your right." class="lightbox-link"></a>
                        <a href="https://jg-photography.s3.eu-west-2.amazonaws.com/lockdown_walks/blackhall-rocks/IMG_20200528_115911.jpg" data-lightbox="walk-seventeen" data-title="The gravel path will bend tomward the right" class="lightbox-link"></a>
                        <a href="https://jg-photography.s3.eu-west-2.amazonaws.com/lockdown_walks/blackhall-rocks/IMG_20200528_115948.jpg" data-lightbox="walk-seventeen" data-title="Stick to the track." class="lightbox-link"></a>
                        <a href="https://jg-photography.s3.eu-west-2.amazonaws.com/lockdown_walks/blackhall-rocks/IMG_20200528_120430.jpg" data-lightbox="walk-seventeen" data-title="Turn left toward the small ravine." class="lightbox-link"></a>
                        <a href="https://jg-photography.s3.eu-west-2.amazonaws.com/lockdown_walks/blackhall-rocks/IMG_20200528_120532.jpg" data-lightbox="walk-seventeen" data-title="Down the steps and cross over." class="lightbox-link"></a>
                        <a href="https://jg-photography.s3.eu-west-2.amazonaws.com/lockdown_walks/blackhall-rocks/IMG_20200528_120555.jpg" data-lightbox="walk-seventeen" data-title="Head up the bank." class="lightbox-link"></a>
                        <a href="https://jg-photography.s3.eu-west-2.amazonaws.com/lockdown_walks/blackhall-rocks/IMG_20200528_120703.jpg" data-lightbox="walk-seventeen" data-title="captioning" class="lightbox-link"></a>
                        <a href="https://jg-photography.s3.eu-west-2.amazonaws.com/lockdown_walks/blackhall-rocks/IMG_20200528_120937.jpg" data-lightbox="walk-seventeen" data-title="Trail ends!" class="lightbox-link"></a>
                        
                        <!-- walks directories
                  
                        img/walks/walk-seventeen
                        https://projects.jonniegrieve.co.uk/walks/img/walk-fourteen
                        walk-fourteen

                        
                       
                        
                      

                        


                        --> 

                    </div> 
                        
                </div>

            </div>

        </div>
    </div>

</article> 